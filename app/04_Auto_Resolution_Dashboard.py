...
st.header('Auto Resolution Dashboard')
tabs = st.tabs(['Human Approval', 'Resolution Status'])

with tabs[0]:
    root_cause_output = st.session_state.get("root_cause_output", {})
    remediation_output = st.session_state.get("remediation_output", {})

    st.subheader("Issue and Solution from Agents")
    if root_cause_output:
        st.write("Root Cause")
        st.json(root_cause_output)
    else:
        st.info("No root cause output found yet. Run the Root Cause Identification Agent first.")

    if remediation_output:
        st.write("Solution")
        st.json(remediation_output)
    else:
        st.info("No remediation output found yet. Run the Remediation Agent first.")

    decision = st.radio("Human Approval", ["Approve", "Reject"], horizontal=True)
    if st.button("Submit Approval"):
        ...
        st.session_state["approval"] = agent_state["approval"]
        st.session_state["human_approval_state"] = agent_state
        st.success(f"Decision recorded: {agent_state['approval']}")


with tabs[1]:
    st.subheader("Human Approval Status")
    approval = st.session_state.get("approval")
    if approval:
        st.success(f"Decision recorded: {approval}")
        st.json(st.session_state.get("human_approval_state", {}))
    else:
        st.info("No human approval decision submitted yet.")
